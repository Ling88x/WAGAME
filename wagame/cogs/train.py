"""Training Grounds — train troops one batch at a time.

`/train` opens an ephemeral panel. Pick a unit from the dropdown (only
...
class TroopSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        view: TrainView = self.view  # type: ignore[assignment]
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message(
                "This isn't your Training Grounds panel.", ephemeral=True
            )
            return False
        return True
...
class TrainCog(commands.Cog):
    ...
    @app_commands.command(name="train", description="Open the Training Grounds panel.")
    async def train(self, interaction: discord.Interaction) -> None:
        ...
        view = TrainView(self.db, interaction.user.id, troops)
    ...
    async def army(self, interaction: discord.Interaction) -> None:
        ...
        if not rows:
            embed.add_field(
                name="No troops yet",
                value="Open `/train` to train your first batch of Catsiths.",
                inline=False,
            )
        else:
