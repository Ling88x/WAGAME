"""/pull — summon hero shards for a specific hero (gacha).

One ephemeral panel per `/pull hero:<name>` invocation. The panel shows
the player's gem balance, progress toward unlock for the target hero,
pity counter, and a Pull button. Clicking Pull spends gems, rolls
shards, persists the result, and edits the embed in place with a
color-coded flash (green on unlock, blue on jackpot, neutral otherwise).
...
async def _fetch_shards(db: Database, user_id: int, hero_id: int) -> tuple[int, int]:
    """Return (count, pity_pulls) for this user+hero; (0, 0) if no row yet."""
    async with db.conn.execute(
...
) -> tuple[PullResult, Flash]:
    """Commit one pull on `hero_row` for `user_id`. Caller pre-checked gems."""
    cost = pull_cost(hero_row["rarity"])
    current, pity = await _fetch_shards(db, user_id, hero_row["id"])
    result = roll_pull(current, pity)
...
def _flash_for_result(result: PullResult, hero_name: str) -> Flash:
    if result.unlocked_now:
        return Flash.ok(
            f"🎉 {hero_name} unlocked! ({result.shards_rolled} shards this pull)"
        )
    if result.pity_activated:
        return Flash.ok(
...
async def _render_embed(
    ...
    embed.add_field(
        name="Pull cost",
        value=f"{pull_cost(hero_row['rarity']):,}",
        inline=True,
    )
    embed.add_field(
        name="Rarity",
    ...
    embed.add_field(
        name="Pity",
        value=f"{pity}/{PITY_PULL_LIMIT} pulls",
        inline=True,
    )
    embed.set_footer(
        text="Every pull yields ≥1 shard. Pity guarantees unlock by the 100th pull."
    )
    return embed
...
class PullView(discord.ui.View):
    ...
    @discord.ui.button(label="Pull", emoji="🔮", style=discord.ButtonStyle.success)
    async def pull(
        ...
        _, flash = await execute_pull(self.db, interaction.user.id, hero_row)
...
class GachaCog(commands.Cog):
    ...
    async def pull(self, interaction: discord.Interaction, hero: str) -> None:
        ...
        view = PullView(self.db, interaction.user.id, int(hero_row["id"]))
        embed = await _render_embed(self.db, interaction.user, hero_row)
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)

    @pull.autocomplete("hero")
