"""Tests for hero summoning logic + the /pull execute path.

Split into:
- pure logic: roll_shards, roll_pull, pity behaviour, expected mean
- DB-backed: execute_pull credits/debits, owned_heroes inserts on unlock
"""
...
from wagame.cogs.gacha import _fetch_shards, execute_pull
from wagame.db import Database
from wagame.game.gacha import (
    DAILY_GEM_BONUS,
    PITY_PULL_LIMIT,
    PULL_COST,
    SHARDS_PER_UNLOCK,
    STARTING_GEMS,
    expected_shards_per_pull,
    pull_cost,
    roll_pull,
    roll_shards,
)
...
async def db(tmp_path: Path) -> Database:
    database = Database(tmp_path / "gacha.db")
...
def test_pull_cost_matches_user_tier_pinning() -> None:
    # Gold / purple / blue costs are the design pillars; lock them in.
    assert PULL_COST["legendary"] == 2000
    assert PULL_COST["epic"] == 900
    assert PULL_COST["rare"] == 800


def test_pull_cost_unknown_rarity_falls_back_to_rare() -> None:
    assert pull_cost("eldritch") == PULL_COST["rare"]


# -- shard distribution ---------------------------------------------------


def test_expected_shards_per_pull_in_target_range() -> None:
    # Between steep (~18) and middle (~27), we tuned to ~20.
    mean = expected_shards_per_pull()
...
def test_pity_caps_pull_count_at_limit() -> None:
    """No combination of unlucky rolls can exceed PITY_PULL_LIMIT without unlock."""
    rng = random.Random(2026)
    # Player who's been spectacularly unlucky: 99 pulls, only 1 shard.
    result = roll_pull(current_shards=1, pity_pulls=PITY_PULL_LIMIT - 1, rng=rng)
...
def test_pity_does_not_clobber_a_naturally_big_roll() -> None:
    ...
    for seed in range(1000):
        ...
        result = roll_pull(current_shards=0, pity_pulls=PITY_PULL_LIMIT - 1, rng=r)
        if result.jackpot:
            assert result.unlocked_now
...
def test_pity_counter_resets_on_unlock() -> None:
    rng = random.Random(7)
    # Pre-stack to 99 shards so any pull >= 1 unlocks.
    result = roll_pull(current_shards=99, pity_pulls=50, rng=rng)
    assert result.unlocked_now
    assert result.new_pity == 0
...
def test_pity_counter_increments_when_no_unlock() -> None:
    ...
    result = roll_pull(current_shards=0, pity_pulls=10, rng=rng)
...
def test_negative_inputs_rejected() -> None:
    with pytest.raises(ValueError):
        roll_pull(current_shards=-1, pity_pulls=0)
    with pytest.raises(ValueError):
        roll_pull(current_shards=0, pity_pulls=-1)


# -- DB-backed execute_pull ------------------------------------------------
...
async def test_execute_pull_debits_gems_and_credits_shards(db: Database) -> None:
    ...
    result, _ = await execute_pull(db, 1, hero_row)
    async with db.conn.execute(
        "SELECT gems FROM players WHERE discord_user_id = 1"
    ) as cur:
        row = await cur.fetchone()
    assert row is not None
    assert int(row["gems"]) == starting_balance - PULL_COST["rare"]

    count, _pity = await _fetch_shards(db, 1, hero_id)
    assert count == result.shards_rolled


async def test_execute_pull_unlocks_hero_on_threshold(db: Database) -> None:
    hero_id = await _seed_hero(db, rarity="rare")
    await db.get_or_create_player(1)
    ...
    async with db.conn.execute("SELECT * FROM heroes WHERE id = ?", (hero_id,)) as cur:
        hero_row = await cur.fetchone()
    assert hero_row is not None
    result, flash = await execute_pull(db, 1, hero_row)
    assert result.unlocked_now
...
async def test_pull_persists_pity_counter(db: Database) -> None:
    hero_id = await _seed_hero(db, rarity="rare")
    await db.get_or_create_player(1)
    async with db.conn.execute("SELECT * FROM heroes WHERE id = ?", (hero_id,)) as cur:
        hero_row = await cur.fetchone()
    ...
    # Burn pulls until either unlock or counter advances. We just verify the
