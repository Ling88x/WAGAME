...
| #16 | UX polish        | ✅ shipped | Hub renamed "Campus", button labels descriptive (Monster Hunting / Resource Gathering / Summon Troops / Your Heroes / Get Heroes / My Profile), summon panel got a ◀/▶ slider through every catalog hero, hero level cap raised to 110, "command/cmd" stat relabeled to "power", several text smoothing passes (gather max, train title, daily quest). |
| #17 | Ghost Arena      | ✅ shipped | Async PvP: a background loop every 12h drafts two enrolled players (≥1 hero, recently active), runs `simulate_battle` from the combat engine seeded on the match id, posts a fight log + ELO swing in each configured server channel. ELO K=32, ratings floor at 100, gems reward 100/50/25. `/arena enroll`, `/arena stats`, `/arena leaderboard`. `/admin set-arena-channel`, `/admin trigger-arena` for setup + manual fires. |
| #18 | Witch's Council  | ✅ shipped | Weekly server-wide quest. One active globally (random kind: kill tenebrals, gather RSS, complete dailies); every player auto-contributes via normal play. Spawn / end announcements post in each configured guild's channel. Completion pays 500 💎 + 3 random hero shards to every ≥1% contributor; top contributor adds a 500-gem bonus. Failed quests pay a consolation. `/council`, hub status + button, `/admin set-council-channel`, `/admin trigger-council`. |
| #19 | Progression feedback | ✅ shipped | Player XP & level rail (cap 100, mirror hero curve, +25 💎 +1 random shard per level-up, title bands Novice/Apprentice/Adept/Mage/Archmage). Hooks across hunt, sighting, gather, daily quest, arena, council. Hunt-kill hero-shard drop (20% at lv1 → 80% cap at lv12+, 1-N shards). Flash messages now always surface hero XP gained + shards dropped + player level-ups; profile shows level + XP bar + title; hub header shows title + level. |
| #20 | Hero Bonds       | ✅ shipped | Per-pair affinity bond table + `/bonds` panel (PR #20 infra). `/hunt` now has a Support hero slot: support contributes 25% atk_eff + 50% command/speed, both heroes earn full kill-XP, the pair accrues bond points every engagement (1/chip, 3/kill × shared-trait affinity). Bond levels add a +5/+10/+20/+35% march-power multiplier at 10/50/150/500 pts. Support hero locked from gathers and other hunts while in flight. |
